...
public class VRTK_UpdatePrompt : EditorWindow
{
    ...
    public void OnGUI()
    {
        ...
        var hidePromptInFuture = GUILayout.Toggle(hideToggle, "Do not prompt for this version again.");
        if (EditorGUI.EndChangeCheck())
        {
            hideToggle = hidePromptInFuture;
            var key = string.Format(hideVersionPrompt, versionReceived);
    ...
    {
        var assetGUID = AssetDatabase.FindAssets(typeof(VRTK_UpdatePrompt).FullName).FirstOrDefault();
        var path = AssetDatabase.GUIDToAssetPath(assetGUID);
        path = Path.GetDirectoryName(Path.GetDirectoryName(path));

        versionLocal = File.ReadAllText(string.Format("{0}/{1}", path, localVersionFile));
    ...
    {
        var versionBits = versionText.Trim().Split('.');
        var formattedVersion = "";
        foreach (var versionBit in versionBits)
        {
    ...
    {
        var changelogBits = fullChangelog.Trim().Split(new[] { "\n# " }, StringSplitOptions.RemoveEmptyEntries);
